-- Fix: start_game could raise "FOREACH expression must not be null" when a
-- team ends up with zero players (e.g. after assign_manual_team moves the
-- last player off a team, leaving the team row behind with no members).
-- array_agg(...) over that team's players returns NULL, and `foreach x in
-- array null_array` is rejected by Postgres. Skip teams with no players
-- when building the turn order instead.
