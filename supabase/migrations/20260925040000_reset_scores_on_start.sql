-- confirm_rematch zeroed team scores the instant the HOST reopened the
-- lobby — but a player who hasn't personally decided yet stays parked on
-- their own final-scoreboard screen (see the client-side gating fix), and
-- that screen reads the same live `teams` row, so it would show "0 - 0"
-- instead of the real final result the moment the host clicked Play Again.
-- Scores should reset when the new game actually starts, not when the
-- lobby merely reopens.
