-- start_game required teams to already exist, but Random mode never forced
-- the host to click "Reshuffle" first — so a host who never touched that
-- button hit a confusing "No teams assigned yet" failure (further hidden by
-- a client-side bug that showed a generic message instead of the real one).
-- In Random mode, just shuffle automatically if nobody has yet. Manual
-- ("Pick") mode still requires explicit picks — there's no sane default to
-- auto-assign there.
